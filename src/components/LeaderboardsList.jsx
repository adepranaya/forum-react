      ...
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {leaderboards.map((leaderboard, idx) => (
            <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-[#222c36] transition-colors group">
              <td className="px-6 py-4 font-bold text-slate-400 group-hover:text-primary">
                #{idx + 1}
              </td>
              <td className="px-6 py-4">
                <div className="flex items-center gap-3">
                  <img src={leaderboard.user.avatar} alt={`User avatar for rank ${idx + 1}`} className="size-8 rounded-full" />
